...
const title = ref("Danh sách bài viết của tôi");
...
const useMoveToTrash = async (id) => {
  ...
  countPublic.value = useCountPublicItem(list)
  countDraft.value = useCountDraftItem(list)
  countMyItem.value = useCountMyItem(list)
};
...
const useFilterPostByCategory = (data) => {
  if (data === 'default') {
    ...
    listFilterSearch.value = list.value.filter(item => item.user_id === $userStore.user.id && item.deleted_at === null && item.category_id === data)
  }
}
</script> 
