...
onMounted(async () => {
  ...
  listTrash.value = listAll.value.filter((item) => item.deleted_at !== null);
  listUser.value = useGetListUserRole($userStore.listUser);

  listPagination.value = listTrash.value.slice(
    (currentPage.value - 1) * itemPerPage.value,
    (currentPage.value - 1) * itemPerPage.value + itemPerPage.value
  );
  totalPage.value = Math.ceil(useCountAllItem(listTrash) / itemPerPage.value);
...
const useRestoreItem = async (data) => {
  $useRestoreItem(data, $categoryStore, $toast);
